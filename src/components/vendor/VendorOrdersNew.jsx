...
const VendorOrders = () => {
  ...
  const updateOrderStatus = async (orderNumber, newStatus, notes = '') => {
    try {
      ...
      // Risk score validation for order confirmation
      if (newStatus === 'confirmed' && order?.securityInfo?.riskScore >= 35) {
  ...
  const getStatusIcon = (status) => {
    ...
    switch (status) {
      ...
      case 'processing': return <PackageIcon className={`${iconClass} text-blue-500`} />;
                    ...
                    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4 border-b border-gray-200">
                      <div className="flex items-center justify-between flex-wrap gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          {getRiskScoreBadge(order.securityInfo?.riskScore)}
                        </div>
